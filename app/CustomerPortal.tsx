...
export default function CustomerPortal({ user }: { user: User }) {
  ...
  return <div className="customer-shell">
    <header className="customer-header">
      <a className="customer-brand" href="#top" aria-label="RK Empires home"><Image src="/rk-empires-logo.jpeg" alt="" width={48} height={48} /><span><strong>RK Empires</strong><small>Intelligent inventory</small></span></a>
    ...
    <main id="top" className="customer-main">
      ...
      {view === "catalog" ? <>
        <section className="customer-hero"><div><p><Sparkles />RK Empires customer portal</p><h1>Order smarter.<br /><span>Move faster.</span></h1><p>Browse live inventory, confirm availability, and place your order in a few focused steps.</p><button onClick={() => document.getElementById("catalog")?.scrollIntoView()}><ShoppingBag />Explore catalog<ChevronRight /></button></div><div className="hero-emblem"><Image src="/rk-empires-logo.jpeg" alt="RK Empires" width={330} height={330} priority /></div></section>
        <section id="catalog" className="catalog-section">
          ...
          {loading ? <div className="customer-loading">Loading current inventory…</div> : <div className="product-card-grid">{filtered.map((product) => <article className="customer-product-card" key={product.id}><div className="product-card-visual"><span style={{ background: product.category_color || "#d4af37" }} /><Package /><small>{product.sku}</small></div><div className="product-card-body"><p>{product.category_name || "General inventory"}</p><h3>{product.name}</h3><span className="product-description">{product.description || `Quality ${product.unit} supplied through RK Empires.`}</span><div className="product-card-meta"><strong>{money(product.price_cents)}</strong><small>{product.stock > 0 ? `${product.stock} ${product.unit}${product.stock === 1 ? "" : "s"} available` : "Out of stock"}</small></div><button disabled={product.stock < 1} onClick={() => { changeQuantity(product, 1); setToast(`${product.name} added to cart`); }}><Plus />{product.stock > 0 ? "Add to cart" : "Unavailable"}</button></div></article>)}{!filtered.length && <div className="catalog-empty"><Package /><strong>No matching products</strong><p>Try another search or category.</p></div>}</div>}
        </section>
      </> : <section className="customer-orders-page"><div className="orders-page-head"><p className="gold-eyebrow">Customer account</p><h1>My orders</h1><p>Review every order placed through your RK Empires account.</p></div><div className="customer-orders-list">{data.orders.map((order) => <article key={order.id}><span><ShoppingBag /></span><div><strong>{order.order_number}</strong><small>{order.item_count} item{Number(order.item_count) === 1 ? "" : "s"} · {formatDate(order.created_at)}</small></div><p><strong>{money(order.total_cents)}</strong><small>{order.status}</small></p></article>)}{!data.orders.length && <div className="orders-empty"><History /><h2>No orders yet</h2><p>Your completed orders will appear here.</p><button onClick={() => setView("catalog")}>Browse the catalog</button></div>}</div></section>}
    </main>
