...
function Modal({ title, description, onClose, children, wide = false }: { title: string; description?: string; onClose: () => void; children: React.ReactNode; wide?: boolean }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
    <section className={`modal ${wide ? "modal-wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <header className="modal-header"><div><p className="eyebrow">StockWise workspace</p><h2 id="modal-title">{title}</h2>{description && <p>{description}</p>}</div><button className="icon-button" onClick={onClose} aria-label="Close dialog"><X size={20} /></button></header>
      {children}
    </section>
  </div>;
}
...
export default function InventoryApp({ user }: { user: User }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
      <div className="brand"><span className="brand-mark"><Boxes size={22} /></span><div><strong>{data.settings.business_name || "StockWise"}</strong><small>Inventory operations</small></div><button className="mobile-close" onClick={() => setMobileNav(false)} aria-label="Close navigation"><X /></button></div>
      ...
      <div className="user-card"><span>{initials(user.displayName)}</span><div><strong>{user.displayName}</strong><small>{user.email}</small></div></div>
    </aside>
...
function SettingsView({ data, saving, onSave, onCategory }: { data: Workspace; saving: boolean; onSave: (payload: Record<string, unknown>) => Promise<unknown>; onCategory: () => void }) {
  const [name, setName] = useState(data.settings.business_name || "StockWise");
